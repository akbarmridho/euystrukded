...
void add_from_food_recipe(ListTree *l, ListFoodRecipe lfr, food_recipe_t fr)
{
    ...
    if (try == NULL) {
        new_empty_tree(fr.food, &search);
    } else {
        search = try;
    }

    for (int i = 0; i < fr.recipe.ingredients_count; i++) {
        int ingredient_id = fr.recipe.ingredients[i];

        boolean ingredient_found = false;
        int k = 0;
        while (k < search->children_count && !ingredient_found) {
            if (search->children[k]->food.id == ingredient_id) {
        ...
        Tree ingredient_tree = lt_search_by_id(l, ingredient_id);

        if (ingredient_tree == NULL) {
            int j = 0;
            boolean found = false;

            while (j < lfr_length(lfr) && !found) {
                if (lfr.contents[j].food.id == ingredient_id) {
                    found = true;
                } else {
                    j++;
                }
            ...
            Tree new_child;
            new_empty_tree(lfr.contents[j].food, &new_child);
            lt_insert_last(l, new_child);
            tree_insert_child(new_child, search);
        } else {
            tree_insert_child(ingredient_tree, search);
        }
    }
}